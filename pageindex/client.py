...
class PageIndexClient:
    ...
    def __init__(
        ...
        if model_api_base:
            overrides["model_api_base"] = model_api_base
        if model_api_key:
            overrides["model_api_key"] = model_api_key
        if retrieve_model:
            overrides["retrieve_model"] = retrieve_model
        if agent_model:
            overrides["agent_model"] = agent_model
        if agent_api_base:
            overrides["agent_api_base"] = agent_api_base
        if agent_api_key:
            overrides["agent_api_key"] = agent_api_key
        opt = ConfigLoader().load(overrides or None)
        self.model = opt.model
        self.retrieve_model = _normalize_retrieve_model(opt.retrieve_model or self.model)
        self.agent_model = getattr(opt, "agent_model", None) or None
        self.agent_api_base = getattr(opt, "agent_api_base", None) or None
        self.agent_api_key = getattr(opt, "agent_api_key", None) or None
        if self.workspace:
